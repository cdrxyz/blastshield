...
4. **Default deny** — If the subcommand isn't read-only, it is treated as mutating. Boolean flags before the subcommand (`terraform --auto-approve apply`, `npm -g install`) do not change this. A space-separated flag value that happens to match a read-only word (`--var plan`, `--namespace get`, `--profile s3_ls`) cannot make a later mutating verb pass.
