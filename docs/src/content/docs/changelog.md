...
- Publishes GitHub releases only from an intentional `v*.*.*` tag or a manual Release workflow dispatch, not on every push to `master`.
- Marks those intentional releases as latest instead of prerelease so GitHub `/releases/latest` resolves.
- Requires the CI workflow to pass in the same run before packaging, checksums, and Homebrew tap dispatch.
